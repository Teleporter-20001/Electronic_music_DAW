...
def generate_waveform(
    ...
    raise WaveGenerateError(f'invalid track type: {type(track)}')
...
def generate_waveform(
    ...
    expected_memory_mb = expected_samples * channels * 8 / (1024 * 1024)    # float64 = 8 bytes
    ...
    for note in track.notes:
        ...
        # single waveform gen
        t = np.linspace(0, duration, int(end_idx - start_idx), endpoint=False)
        ...
        if start_idx < 0 or end_idx > waveform.shape[0]:
            raise WaveGenerateError(f'Index out of range: start index {start_idx}, end index {end_idx}, invalid range (0, waveform.shape[0])')
